...
organization TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_post_details_post_id ON post_details(post_id, id);
  ...
  // Structured Important Dates / Application Fee fields for existing databases.
  // Older databases do not have these columns, so add them during startup.
  const refreshedPostColumns = db.prepare('PRAGMA table_info(posts)').all().map(column => column.name);
  if (!refreshedPostColumns.includes('important_dates')) {
    db.exec('ALTER TABLE posts ADD COLUMN important_dates TEXT');
  }
  if (!refreshedPostColumns.includes('application_fees')) {
    db.exec('ALTER TABLE posts ADD COLUMN application_fees TEXT');
  }
  if (!refreshedPostColumns.includes('update_details')) {
    db.exec('ALTER TABLE posts ADD COLUMN update_details TEXT');
  }

  const legacyVacancies = db.prepare(`
    SELECT id, title, total_vacancies, qualification, age_limit
    FROM posts
    WHERE post_type = 'vacancy'
      AND NOT EXISTS (SELECT 1 FROM post_details WHERE post_details.post_id = posts.id)
  `).all();
  const migrateDetail = db.prepare(`
    INSERT INTO post_details (post_id, post_name, seats, age_limit, qualification)
    VALUES (?, ?, ?, ?, ?)
  `);
  for (const post of legacyVacancies) {
    migrateDetail.run(post.id, post.title || 'Post', post.total_vacancies || 'Not specified', post.age_limit || '', post.qualification || '');
  }
...
function getPostDetails(postId) {
  return db.prepare(`SELECT id, post_name, seats, age_limit, qualification FROM post_details WHERE post_id = ? ORDER BY id ASC`).all(postId);
}

function clientPost(row) {
  return { ...row, notification_pdf: row.notification_pdf ? `/uploads/${path.basename(row.notification_pdf)}` : null, post_details: getPostDetails(row.id) };
}

function validatePost(input) {
  const postType = plain(input.post_type, 20);
  const category = (plain(input.category, 20) || 'other').toLowerCase();
...
important_note: multiline(input.important_note, 3000)
  ...
  const rawDetails = Array.isArray(input.post_details) ? input.post_details : [];
  const postDetails = rawDetails.map((item) => ({
    post_name: multiline(item?.post_name, 250),
    seats: multiline(item?.seats, 200),
    age_limit: multiline(item?.age_limit, 200),
    qualification: multiline(item?.qualification, 1000)
  })).filter(item => item.post_name || item.seats || item.age_limit || item.qualification);

  if (postType === 'vacancy' && !postDetails.length) return { error: 'Add at least one post with seats, age limit and qualification details.' };
  for (const item of postDetails) {
    if (!item.post_name || !item.seats || !item.qualification) return { error: 'Each post row must include Post Name, No. of Seats and Qualification.' };
  }
  data.post_details = postDetails;
  ...
  if (postDetails.length) {
    data.total_vacancies = postDetails.length === 1 ? postDetails[0].seats : `${postDetails.length} posts`;
    data.qualification = postDetails.length === 1 ? postDetails[0].qualification : 'See post-wise qualification details';
    data.age_limit = postDetails.length === 1 ? (postDetails[0].age_limit || null) : 'See post-wise age limits';
  }

  for (const field of ['application_start_date', 'last_date', 'event_date']) {
    data[field] = optional(input[field], 250);
  }

  const structuredDates = Array.isArray(input.important_dates) ? input.important_dates.map(item => ({
    event: multiline(item?.event, 120),
    value: multiline(item?.value, 250)
  })).filter(item => item.event || item.value) : [];

  const structuredFees = Array.isArray(input.application_fees) ? input.application_fees.map(item => ({
    category: multiline(item?.category, 120),
    value: multiline(item?.value, 250)
  })).filter(item => item.category || item.value) : [];

  for (const item of structuredDates) {
    if (!item.event || !item.value) return { error: 'Each Important Dates row must include Event and Date / Information.' };
  }
  for (const item of structuredFees) {
    if (!item.category || !item.value) return { error: 'Each Application Fee row must include Category and Fee / Information.' };
  }

  data.important_dates = JSON.stringify(structuredDates);
  data.application_fees = JSON.stringify(structuredFees);

  const updateDetails = Array.isArray(input.update_details) ? input.update_details.map(item => ({
    field: multiline(item?.field, 120),
    value: multiline(item?.value, 500)
  })).filter(item => item.field || item.value) : [];
  for (const item of updateDetails) {
    if (!item.field || !item.value) return { error: 'Each update-detail row must include Field and Information.' };
  }
  data.update_details = JSON.stringify(updateDetails);

  // Keep legacy fields synchronized for existing cards/API consumers.
  const startDate = structuredDates.find(item => item.event.toLowerCase() === 'application start');
  const lastDate = structuredDates.find(item => item.event.toLowerCase() === 'last date');
  const examDate = structuredDates.find(item => item.event.toLowerCase() === 'exam');
  const generalFee = structuredFees.find(item => item.category.toLowerCase() === 'general/obc') || structuredFees[0];
  data.application_start_date = startDate?.value || data.application_start_date || null;
  data.last_date = lastDate?.value || data.last_date || null;
  data.event_date = examDate?.value || data.event_date || null;
  data.application_fee = generalFee?.value || data.application_fee || null;
  ...
  if (postType === 'vacancy') {
    for (const field of ['application_start_date', 'last_date', 'application_fee', 'apply_link']) if (!data[field]) return { error: 'Complete all required Vacancy fields.' };
...
const resetLink = `${baseUrl.replace(/\/$/, '')}/reset-password.html?token=${encodeURIComponent(token)}`;
  ...
  const newPostId = Number(row.lastInsertRowid);
  const detailInsert = db.prepare(`INSERT INTO post_details (post_id, post_name, seats, age_limit, qualification) VALUES (?, ?, ?, ?, ?)`);
  for (const item of d.post_details) detailInsert.run(newPostId, item.post_name, item.seats, item.age_limit || null, item.qualification);
  ...
  return sendJson(response, 201, newPost);
    ...
    if (match) {
      ...
      if (request.method === 'PUT') {
        ...
        const d = result.data;
        const fields = ['post_type','category','organization','title','total_vacancies','qualification','age_limit','application_start_date','last_date','application_fee','apply_link','notification_pdf','content_link','event_date','important_dates','application_fees','update_details','description','selection_process','how_to_apply','important_note'];
        const update = db.prepare(`UPDATE posts SET ${fields.map(f => `${f} = ?`).join(', ')}, updated_at = ? WHERE id = ?`);
        const outcome = update.run(...fields.map(f => d[f]), new Date().toISOString(), id);
        if (!outcome.changes) return sendError(response, 404, 'Post not found.');
        db.prepare('DELETE FROM post_details WHERE post_id = ?').run(id);
        const detailInsert = db.prepare(`INSERT INTO post_details (post_id, post_name, seats, age_limit, qualification) VALUES (?, ?, ?, ?, ?)`);
        for (const item of d.post_details) detailInsert.run(id, item.post_name, item.seats, item.age_limit || null, item.qualification);
        return sendJson(response, 200, clientPost(db.prepare('SELECT * FROM posts WHERE id = ?').get(id)));
      }
      if (request.method === 'DELETE') {
        const row = db.prepare('SELECT notification_pdf FROM posts WHERE id = ?').get(id);
        if (!row) return sendError(response, 404, 'Post not found.');
        db.prepare('DELETE FROM posts WHERE id = ?').run(id);
        if (row.notification_pdf) { const file = path.join(ROOT, row.notification_pdf); if (file.startsWith(UPLOAD_DIR) && fs.existsSync(file)) fs.unlinkSync(file); }
