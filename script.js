...
function openModal(post) {
  ...
  const updateDetails = parseStructured(post.update_details);

  const updateRows = updateDetails.length
    ? updateDetails.map(item => `<tr><td>${esc(item.field || '')}</td><td><strong>${esc(item.value || '')}</strong></td></tr>`).join('')
    : '';

  const updateConfig = {
    'admit-card': {
      eyebrow: 'EXAM UPDATE',
      heading: 'ADMIT CARD DETAILS',
      dateTitle: 'IMPORTANT DATE',
      dateLabel: 'Admit Card / Exam Date',
      linkLabel: 'DOWNLOAD ADMIT CARD',
      linkText: 'Download Admit Card',
      sectionClass: 'section-admit'
    },
    result: {
      eyebrow: 'EXAM UPDATE',
      heading: 'RESULT DETAILS',
      dateTitle: 'RESULT DATE',
      dateLabel: 'Result Date',
      linkLabel: 'RESULT LINK',
      linkText: 'View Result',
      sectionClass: 'section-result'
    },
    'answer-key': {
      eyebrow: 'EXAM UPDATE',
      heading: 'ANSWER KEY DETAILS',
      dateTitle: 'RELEASE DATE',
      dateLabel: 'Answer Key Release',
      linkLabel: 'ANSWER KEY LINK',
      linkText: 'View Answer Key',
      sectionClass: 'section-answer'
    }
  };

  if (updateConfig[post.post_type]) {
    const cfg = updateConfig[post.post_type];
    const dateValue = post.event_date ? date(post.event_date) : '';
    const infoSection = updateRows ? `
      <section class="detail-section full-width update-info-section ${cfg.sectionClass}">
        <h3><span class="section-icon">▣</span> ${cfg.heading}</h3>
        <table class="detail-table compact-table update-details-table">
    ...
    ` : `
      <section class="detail-section full-width update-info-section ${cfg.sectionClass}">
        <h3><span class="section-icon">▣</span> ${cfg.heading}</h3>
        <p class="detail-empty">No additional update information available.</p>
      </section>
    `;
    ...
    const dateBlock = `
      <section class="detail-section section-dates">
        <h3><span class="section-icon">▦</span> ${cfg.dateTitle}</h3>
        ${dateValue ? `<table class="detail-table compact-table"><thead><tr><th>Event</th><th>Date / Information</th></tr></thead><tbody><tr><td>${cfg.dateLabel}</td><td><strong>${esc(dateValue)}</strong></td></tr></tbody></table>` : '<p class="detail-empty">No date information available.</p>'}
      </section>
    `;

    const linkBlock = post.content_link ? `
      <section class="detail-section section-links">
        <h3><span class="section-icon">↗</span> ${cfg.linkLabel}</h3>
        <table class="detail-table compact-table links-table"><thead><tr><th>Link</th><th>Action</th></tr></thead><tbody><tr><td>${cfg.linkLabel}</td><td><a href="${esc(post.content_link)}" target="_blank" rel="noopener">${cfg.linkText}</a></td></tr></tbody></table>
      </section>
    ` : '';

    const note = post.important_note ? `
      <section class="detail-section section-note"><h3><span class="section-icon">!</span> IMPORTANT NOTE</h3><div class="detail-description">${esc(post.important_note)}</div></section>
    ` : '';

    details.innerHTML = `
      ${intro}
      ${infoSection}
      <div class="detail-columns">${dateBlock}${linkBlock}</div>
      ${post.how_to_apply ? `<section class="detail-section section-apply"><h3><span class="section-icon">▤</span> INSTRUCTIONS</h3><div class="detail-description">${esc(post.how_to_apply)}</div></section>` : ''}
      ${note}
    `;
    links.innerHTML = '';
    modal.showModal();
  ...
  const importantDates = parseStructured(post.important_dates);
  const dateRows = importantDates.length
    ? importantDates.map(item => `<tr><td>${esc(item.event || '')}</td><td><strong>${esc(item.value || '')}</strong></td></tr>`).join('')
    : [
        post.application_start_date ? `<tr><td>Application Start</td><td><strong>${esc(date(post.application_start_date))}</strong></td></tr>` : '',
        post.last_date ? `<tr><td>Last Date</td><td><strong>${esc(date(post.last_date))}</strong></td></tr>` : '',
        post.event_date ? `<tr><td>Exam</td><td><strong>${esc(date(post.event_date))}</strong></td></tr>` : ''
      ].join('');
    ...
    <section class="detail-section section-dates">
      ...
      ${dateRows ? `
        <table class="detail-table compact-table">
          <thead><tr><th>Event</th><th>Date / Information</th></tr></thead>
  ...
  const applicationFees = parseStructured(post.application_fees);
  const feeRows = applicationFees.length
    ? applicationFees.map(item => `<tr><td>${esc(item.category || '')}</td><td>${esc(item.value || '')}</td></tr>`).join('')
    : post.application_fee
      ? `<tr><td>Application Fee</td><td>${esc(post.application_fee)}</td></tr>`
      : '';
  ...
  const feeSection = `
    <section class="detail-section section-fee">
      ...
      ${feeRows ? `
        <table class="detail-table compact-table">
          <thead><tr><th>Category</th><th>Fee / Information</th></tr></thead>
          <tbody>${feeRows}</tbody>
        </table>
  ...
  const ageSection = post.post_details?.length ? '' : `
  ...
  const eligibilitySection = post.post_details?.length ? '' : `
  ...
  const postDetails = Array.isArray(post.post_details) ? post.post_details : [];
    ...
    <section class="detail-section full-width section-vacancy">
      ...
      ${postDetails.length ? `
        <div class="detail-table-scroll">
          <table class="detail-table compact-table vacancy-table post-wise-table">
            <thead><tr><th>Post Name</th><th>No. of Seats</th><th>Age Limit</th><th>Qualification</th></tr></thead>
            <tbody>${postDetails.map(item => `
              <tr><td>${esc(item.post_name || '')}</td><td>${esc(item.seats || 'Not specified')}</td><td>${esc(item.age_limit || 'As per notification')}</td><td>${esc(item.qualification || 'As per notification')}</td></tr>
            `).join('')}</tbody>
          </table>
        </div>
      ` : `
        <table class="detail-table compact-table vacancy-table">
          <thead><tr><th>Organization</th><th>Post Name</th><th>${post.post_type === 'other' ? 'Seats' : 'Total Vacancies'}</th></tr></thead>
          <tbody><tr><td>${esc(post.organization || '')}</td><td>${esc(post.title || '')}</td><td>${esc(post.total_vacancies || 'Not specified')}</td></tr></tbody>
        </table>
      `}
    </section>
  `;
  ...
  details.innerHTML = `
    ...
    ${selectionSection || howToApplySection || linksSection ? `<div class="detail-columns detail-bottom-grid">${selectionSection}${howToApplySection}${linksSection}</div>` : ''}
